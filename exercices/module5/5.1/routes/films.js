...
/* Read All films . */
router.get('/', (req, res) => {
  const filteredByDuration = req?.query ? Number(req.query['minimum-duration']) : undefined;

  console.log('Filter : ', filteredByDuration);
  console.log('type ', typeof filteredByDuration);

  
  if (filteredByDuration && (typeof filteredByDuration !== 'number' || filteredByDuration <= 0))
    return res.sendStatus(400);

  if (!filteredByDuration) return res.json(MOVIES);

  const filteredMovies = [...MOVIES].filter((film) => film.duration >= filteredByDuration);
