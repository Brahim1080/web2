...
function readAllMovies(filtered) {
  ...
  if (filtered !== undefined && (isNaN(filtered) || filtered <= 0))
    return res.sendStatus(400);

  const movies = parse(jsonDbPath, MOVIES);
  if (!filtered) return movies;

  return [...movies].filter((film) => film.duration >= filtered);
}
...
function updateOneFilm(id, propertiesToUpdate) {
  ...
  if (foundIndex < 0) return undefined;
  const propertiesToUpdateEscapeHtml = {
    title: escape(propertiesToUpdate.title),
    duration,
    budjet,
    link: escape(propertiesToUpdate.link),
  };
  console.table({ propertiesToUpdate, propertiesToUpdateEscapeHtml });
  console.log(propertiesToUpdateEscapeHtml.title);
  const updatedFilm = { ...movies[foundIndex], ...propertiesToUpdateEscapeHtml };
